...
vec4 textImage(vec4 fragCoord);

void main(void) {
    vec4 col;
    col = mainImage(gl_FragCoord.xy);

    if (time > 13700)
        col += textImage(gl_FragCoord);
    color = col;
}                                  
...
vec4 mainImage(vec2 FragCoord )
{
    ...
    if (resolution.x>1500.&&abs(uv.y)>.37) return fragColor;
...
float character(vec4 ch, vec2 uv)
{
...
{
    float col = 0.0;
    float t= mod(time-14000,38.);// scroll duration 
    vec2 center = vec2(40.,1.);
    
    print_pos = (vec2(300.-t*30.0,21.0) - vec2(STRWIDTH(1.0),STRHEIGHT(1.0))/2.0);
 
    col += character(ch_per,uv);
    col += character(ch_per,uv);
    col += character(ch_per,uv);
    
    col += character(ch_W,uv);
    col += character(ch_E,uv);
    col += character(ch_spc,uv);
    col += character(ch_P,uv);
    col += character(ch_R,uv);
    col += character(ch_O,uv);
    col += character(ch_U,uv);
    col += character(ch_D,uv);
    col += character(ch_L,uv);
    col += character(ch_Y,uv);
    col += character(ch_spc,uv);

    col += character(ch_P,uv);
    col += character(ch_R,uv);
    col += character(ch_E,uv);
    col += character(ch_S,uv);
    col += character(ch_E,uv);
    col += character(ch_N,uv);
    col += character(ch_T,uv);
    col += character(ch_spc,uv);
  
	col += character(ch_T,uv);
    col += character(ch_H,uv);
    col += character(ch_E,uv);
    col += character(ch_spc,uv);
    
    col += character(ch_B,uv);
    col += character(ch_R,uv);
    col += character(ch_A,uv);
    col += character(ch_N,uv);
    col += character(ch_D,uv);
    col += character(ch_spc,uv);
    ...
    col += character(ch_spc,uv);
    col += character(ch_O,uv);
    col += character(ch_F,uv);
    col += character(ch_C,uv);
    col += character(ch_spc,uv);
    col += character(ch_exc,uv);
    col += character(ch_exc,uv);
    col += character(ch_exc,uv);
...
vec4 textImage(vec4 fragCoord) 
{
    ...
    vec2 px = fragCoord.xy / resolution.xy*2.-1.;
    ...
    /// text layer !!
    vec2 vt = 1024.*fragCoord.xy / resolution.xy;
